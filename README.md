...
### Automatic build on commit (Git hook)

The repository ships a versioned `pre-commit` hook in the */.githooks* directory.
It runs `npm run build` in the */frontend* directory on every commit, aborts the
commit if the build fails, and stages the regenerated */docs* output.
...
Both scripts simply set `git config core.hooksPath .githooks`.
Building the frontend requires the dependencies, so run `npm install` in
*/frontend* first.
