...
class ChatEntry(TypedDict):
    ...
    # Marks the start of the current scene. When set, the history may be read
    # only from this message onwards (excluding the message itself). Absent
    # unless the user explicitly flagged the message.
    last_scene_message: NotRequired[bool]
...
class ChatRepository(Repository):
    ...
    def add_user_message(
        ...
        """
        return self.__add_chat_entry("user", content, virtual_ts=virtual_ts)

    def add_tool_message(self, content: str, func_name: str, tool_call_id: str) -> int:
        """
        ...
        """
        return self.__add_chat_entry("tool", content, func_name, tool_call_id)

    def add_agent_message(self, content: str, virtual_ts: Optional[int] = None) -> int:
        """
        ...
        """
        return self.__add_chat_entry("agent", content, virtual_ts=virtual_ts)

    def add_director_message(self, content: str, virtual_ts: int) -> int:
        ...
        """
        return self.add_agent_message(f"<director>\n{content}\n</director>", virtual_ts)
    ...
    def set_last_scene_message(self, timestamp: int, value: bool) -> bool:
        """Sets or clears the ``last_scene_message`` flag of a message identified by its timestamp.

        The flag marks the boundary from which the current scene's history is
        read. Setting it to ``False`` removes the key entirely so the stored
        entries stay free of redundant ``False`` values.

        Args:
            timestamp: The timestamp of the message to update.
            value: ``True`` to mark the message as the scene boundary, ``False``
                to clear the flag.
        ...
        for message in self.data:
            if message.get("timestamp") == timestamp:
                if value:
                    message["last_scene_message"] = True
                else:
                    message.pop("last_scene_message", None)
                self.save_changes()
                return True
        return False
    ...
    def delete_all_messages(self) -> int:
        """
        Removes every message from the chat history.

        Clears the entire history in one step and persists the now-empty log.
