...
class LLMService:
    ...
    def send_messages(
        ...
        **kwargs
        ...
        # The chat template renders a tool result only from the ``tool_calls`` of
        # the preceding assistant message, resolving each ``tool_call_id`` to its
        # function name. Copying just role and content would therefore silently
        # drop every tool response from the prompt, so the tool linkage fields are
        # forwarded whenever the history entry carries them.
        for message in chat_messages:
            entry = {"role": message["role"], "content": message["content"]}
            for key in ("tool_calls", "tool_call_id", "name"):
                value = message.get(key)  # type: ignore[attr-defined]
    ...
    def strip_tool_call(self, content: str) -> str:
        """Remove the raw tool call markup from a model response.

        The call is replayed to the model through the structured ``tool_calls``
        field of the assistant message, so leaving the markup in the text content
        as well would emit it a second time into the prompt.
        """
