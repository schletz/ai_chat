...
from schemas import AssistantMessageInsert, AssistantMessageRequest, ChatMessage, ChatMessageUpdate, DirectorMessage, LastSceneMessagePatch, PlotGenerateRequest, PlotMessage, SummaryGenerateRequest
...
async def generate_director_plot(
    ...
    the current context via a dedicated LLM call and stores it as the
    character's plot, overwriting any previously stored one. No assistant turn
    is produced; the plot only shapes the next generated assistant responses by
    being appended to their system prompt. Because the director inference runs
    here, the LLM lock is required.
...
@router.post("/{name}/chat/summary/generate", status_code=status.HTTP_201_CREATED)
async def generate_summary(
    name: str,
    summary_req: SummaryGenerateRequest,
    char_repo: CharacterRepository = Depends(get_character_repository),
    agent: CharacterAgent = Depends(get_character_agent),
    llm_lock: asyncio.Lock = Depends(get_llm_lock)
):
    """
    Summarizes the current session and appends the summary to the chat history.

    The agent condenses the session context into a recap and stores it as an
    agent message wrapped in ``<summary>`` tags. The entry preceding it becomes
    the scene boundary, so subsequent turns are inferred from the summary
    onwards instead of the full history, which shrinks the context while the
    roleplay continues seamlessly. Nothing is deleted. Because the summary
    inference runs here, the LLM lock is required.

    Args:
        name: The unique identifier for the character.
        summary_req: The summary payload. Includes the mandatory
            ``base_timestamp`` (milliseconds since epoch) representing the
            current in-world time. Always supplied by the frontend.
        char_repo: Dependency-injected character repository.
        agent: Dependency-injected character agent.
    ...
    Returns:
        A confirmation message together with the generated summary text.
    ...
    async with llm_lock:
        summary = await asyncio.to_thread(
            agent.summarize, summary_req.base_timestamp
        )

    return {"message": "Summary added.", "summary": summary}
