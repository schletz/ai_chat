...
class ChatHistory:
    ...
    def build_system_prompt(self,
        ...
        Args:
            include_intro: Appends the character's intro in an ``<intro>`` block.
            include_plot: Appends the character's stored plot in a ``<plot>``
                block after the intro, so the prompt reads
                system prompt -> intro -> plot.
        ...
        if include_plot:
            plot = self.character_repo.get_plot(self.character).strip()
            # Characters without a stored plot keep the previous prompt layout.
            if plot:
                prompt += "\n\n<plot>\n" + plot + "\n</plot>"
    ...
    def build_timestamp_metadata(self, base_timestamp: Optional[int] = None, with_elapsed_time: bool = True) -> str:
        ...
        if with_elapsed_time:
            ...
            ago_str = self.__build_ago_str(last_timestamp, current_timestamp, "after last user message")
    ...
    def get_filtered_chat_history(
        ...
        Args:
            ...
            current_scene_only: When True, the history is read only from the most
                recent message flagged with ``last_scene_message`` onwards. The
                flagged message itself is excluded. Because the history is walked
                backwards, the newest flagged message wins automatically. This
                boundary takes precedence over ``cap_history``: if the flagged
                message is reached first, the walk stops there.
        ...
        if cap_history is None and not current_scene_only:
            ...
            for msg in reversed(raw_history):
                # The scene boundary has precedence over cap_history: stop before
                # the flagged message and exclude it. Checked on the raw entry so
                # a flagged message dropped by the filters still acts as boundary.
                if current_scene_only and msg.get("last_scene_message", False):
                    break
                cleaned = clean_entry(msg)
                if cleaned is not None:
                    clean_history.append(cleaned)
                    if cap_history is not None and len(clean_history) >= cap_history:
                        break
    ...
    def __inject_sticky_note(self, clean_history: list[ChatEntry], base_timestamp: int) -> None:
        ...
        *user* messages (agent-flagged or not) must follow the note in the
        ...
        if pos <= 0:
            ...
            # Walk backwards counting user messages; insert right after the
    ...
    def get_history_text(
        ...
        summary_str = ""
        current_timestamp = base_timestamp / 1000 if base_timestamp else 0
        while i >= 0:
            message = history[i]
            role = f"{user_name.upper()}" if message["role"] == "user" else f"{character_name.upper()}"
            ago_str = ""
            if current_timestamp:
                # Find last user message for timestamp
                last_user_idx = i
                while last_user_idx >= 0:
                    if history[last_user_idx]["role"] == "user":
                        break
                    last_user_idx -= 1

                virtual_ts = history[last_user_idx].get("virtual_ts", 0) / 1000 if last_user_idx >= 0 else 0
                if virtual_ts:
                    ago_str = self.__build_ago_str(virtual_ts, current_timestamp, "ago")
            content = whitespace_regex.sub(' ', message["content"])
            content = clean_regex.sub('', content)
            summary_str = f"[{role}{ago_str}] {content}\n{summary_str}"
            i -= int(distance)
            distance *= factor
    ...
    def get_context(self, include_system_prompt: bool = True,
        ...
        """
        intro = self.character_repo.get_intro(self.character) if include_intro else ""
        history = self.get_filtered_chat_history(
    ...
    def __build_ago_str(self, message_timestamp: float, base_timestamp: float,  text:str) -> str:
        ago_str = ""
        prev_message_ago = base_timestamp - message_timestamp
        if prev_message_ago >= 24 * 3600:
            days = prev_message_ago // 86400
            ago_str = f" ({days} days {text})"
