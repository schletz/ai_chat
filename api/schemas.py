...
class SummaryGenerateRequest(BaseModel):
    """Schema definition for triggering a summary of the current session.

    ``base_timestamp`` (milliseconds since epoch) is mandatory: it dates the
    elapsed-time hints of the summarized context and is recorded as the created
    summary entry's ``virtual_ts``.
    """
...
class CharacterAnswerCountPatch(BaseModel):
    """Schema definition for patching only the per-character answer count."""
