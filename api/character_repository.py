...
class CharacterRepository(Repository):
    ...
    def get_full_history_for_last_n(self, character: str) -> Optional[int]:
        ...
        """
        return self.__get_full_history_for_last_n_value(character, "full_history_for_last_n")
    ...
    def get_full_history_for_last_n_plot_generation(self, character: str) -> Optional[int]:
        ...
        """
        value = self.__get_full_history_for_last_n_value(character, "full_history_for_last_n_plot_generation")
        if value is not None:
            return value
        return self.get_full_history_for_last_n(character)

    def __get_full_history_for_last_n_value(self, character: str, key: str) -> Optional[int]:
        """Read a non-negative integer history-window setting for a character.

        Shared by the roleplay and plot-generation getters; returns ``None`` when
        the character is unknown or the stored value is missing or invalid.
        """
        found = self.get_character_by_name(character)
        if not found:
            return None
    ...
    def get_generation_params(self, character: str) -> dict:
        ...
        params = {}
        for key in ["temperature", "min_p", "top_p", "repeat_penalty", "frequency_penalty", "presence_penalty", "max_tokens"]:
    ...
    def set_character_settings(
        ...
        **kwargs
        ...
        generation_params = ["temperature", "min_p", "top_p", "repeat_penalty", "frequency_penalty", "presence_penalty", "max_tokens"]
        ...
        for key, value in kwargs.items():
            if key in ("character_name", "user_name") and value is not None:
                ...
                if value is not None and value > 0:
                    ...
                    found.pop(key, None)
            elif key in ("full_history_for_last_n", "full_history_for_last_n_plot_generation"):
