...
export class CharacterFormService {
    ...
    /**
     * Updates only the number of assistant answers generated per request.
     * @param name - The name of the character to update.
     * @param answerCount - The new number of answers per request (at least 1).
     */
    async updateAnswerCount(name: string, answerCount: number): Promise<void> {
        await api.patch(`/characters/${name}/answer-count`, {
