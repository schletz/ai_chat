...
/**
 * Processes raw message content by escaping HTML, applying basic markdown
 * formatting and splitting the text into paragraphs. A blank line separates
 * paragraphs; single newlines stay as line breaks and are rendered by the
 * pre-wrap white-space rule.
 */
const formattedContent = computed(() => {
  ...
  // Wrap each paragraph in its own element so CSS controls the gap between
  // them. Joined without a separator because pre-wrap would render any
  // whitespace between the tags as extra blank space.
  return text
    .split(/\n[^\S\n]*\n\s*/)
...
/* Paragraphs carry no default margin; the gap between them is a little more
   than half a line height - clearly visible, but cheaper than a blank line. */
.msg-content :deep(p) {
  margin: 0;
}
