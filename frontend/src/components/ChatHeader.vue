<template>
  <div class="header">
    ...
    <ChatMenu v-if="showMenu" :append-timestamp="appendTimestamp" :enable-thinking="enableThinking"
      ...
      @update-answer-count="$emit('update-answer-count', $event)"
      ...
      @cancel-summary="$emit('cancel-summary')"
...
const props = defineProps<{
  ...
  /** Number of assistant answers per request, forwarded to the submenu number field. */
  answerCount: number;
  ...
  /** Whether the history is currently being summarized, driving the menu spinner. */
...
// Plot generation and summarizing keep the submenu open so their inline spinner
// stays visible while the backend works. The watcher below dismisses the menu
// once the running operation finishes.
watch(
  () => props.isGeneratingPlot || props.isSummarizing,
  (running, wasRunning) => {
