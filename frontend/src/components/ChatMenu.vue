<template>
  ...
  <div class="header-menu" role="menu">
    ...
    <label class="menu-item menu-number" title="Anzahl der Antworten, die pro Anfrage hintereinander erzeugt werden">
      <span>Antwortanzahl</span>
      <input type="number" min="1" step="1" :value="answerCount"
        @change="onAnswerCountChange" />
    </label>
    ...
    <button type="button" class="menu-item"
      :class="{ 'menu-item-cancel': isSummarizing }"
      :title="isSummarizing ? 'Zusammenfassung abbrechen' : 'History zusammenfassen und den Kontext verkleinern'"
      @click="isSummarizing ? $emit('cancel-summary') : $emit('generate-summary')">
      <span class="menu-icon">{{ isSummarizing ? '🛑' : '📝' }}</span>
      <span>{{ isSummarizing ? 'Zusammenfassen abbrechen' : 'Zusammenfassen' }}</span>
      <span v-if="isSummarizing" class="menu-spinner" aria-hidden="true"></span>
    </button>
...
defineProps<{
  ...
  /** Number of assistant answers generated per request, shown in the number field. */
  answerCount: number;
  ...
  /** Whether the history is currently being summarized, showing an inline spinner. */
...
const emit = defineEmits<{
  ...
  /** Carries the desired new answer count after the user changed the number field. */
  (e: "update-answer-count", value: number): void;
  (e: "toggle-debug"): void;
  ...
  /** Requests a summary of the history, shrinking the context of the following turns. */
  (e: "generate-summary"): void;
  /** Requests aborting an in-flight summary generation. */
  (e: "cancel-summary"): void;
  (e: "open-context"): void;
...
/**
 * Normalizes the raw number-input value and reports it to the parent. Empty or
 * invalid entries fall back to a single answer, which is the backend default.
 *
 * @param event - The change event of the number input.
 */
const onAnswerCountChange = (event: Event) => {
  const parsed = Number.parseInt((event.target as HTMLInputElement).value, 10);
  emit("update-answer-count", Number.isNaN(parsed) || parsed < 1 ? 1 : parsed);
};
</script>
...
/* The answer-count row reuses the menu-item layout but trails a number input
   pushed to the right edge of the menu. */
.menu-number {
  user-select: none;
