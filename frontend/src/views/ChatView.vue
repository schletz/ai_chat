<template>
  <ChatHeader :name="name" :base-timestamp="baseTimestamp" :formatted-virtual-time="formattedVirtualTime"
    ...
    @toggle-thinking="onEnableThinkingChange" @update-answer-count="onAnswerCountChange"
    ...
    @generate-summary="summarizeHistory" @cancel-summary="cancelSummary"
...
// Mirrors the character's answer_count setting: how many assistant answers a
// single request generates. Changing it persists via the PATCH endpoint.
...
// True while the history is being summarized, so the burger menu can show an inline spinner.
...
const initChat = async () => {
  try {
    ...
    if (character) {
      ...
      // A missing count means a single answer per request.
      answerCount.value = character.answer_count ?? 1;
...
/**
 * Persists the answer count entered in the header menu. The value is read at
 * inference time and decides how many answers a single request produces.
 * Reverts the local state if the request fails so the UI stays in sync.
 *
 * @param newValue The desired number of answers per request.
 */
const onAnswerCountChange = async (newValue: number) => {
  const previous = answerCount.value;
  answerCount.value = newValue;
  try {
    await formService.updateAnswerCount(props.name, newValue);
  } catch (error) {
    console.error("Failed to update answer_count", error);
    answerCount.value = previous;
  }
};
...
/**
 * Triggers backend summarization of the session. The agent appends the summary
 * to the history and marks the entry before it as the scene boundary, so the
 * following turns run on the shrunken context while the roleplay continues.
 */
const summarizeHistory = async () => {
  isSummarizing.value = true;
  try {
    await chatService.generateSummary(currentVirtualTimestamp.value);
    await loadChat();
  } catch (error) {
...
/**
 * Aborts an in-flight summarization by resetting the server-side KV cache,
 * which makes the backend stop the current turn immediately. The pending
 * summarizeHistory call then settles and clears the summarizing state.
 */
const cancelSummary = async () => {
  try {
    await systemService.resetKvCache();
  } catch (error) {
    console.error("Failed to cancel summarization", error);
  }
};
