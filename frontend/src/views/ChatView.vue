<template>
  <ChatHeader :name="name" :base-timestamp="baseTimestamp" :formatted-virtual-time="formattedVirtualTime"
    ...
    @generate-director-plot="generateDirectorPlot" @open-context="openContextPanel"
    @reset-kv-cache="resetKvCache" />
  ...
  <div class="content chat-window" ref="chatContainer">
    <template v-for="(msg, index) in messages" :key="msg.timestamp">
      ...
      <ChatMessageBubble v-else :message="msg" @delete="deleteMsg" @edit="editMsg" />
    </template>
  </div>
    ...
    <div v-if="messages.length > 0" class="action-menu-wrapper">
      ...
      <template v-if="showActionsMenu">
        ...
        <div class="action-menu" role="menu">
          <AsyncButton :action="() => runActionAndClose(recreate)" customClass="action-menu-item">
...
const loadChat = async () => {
  ...
  const history = await chatService.getHistory(
    currentVirtualTimestamp.value, summaryBaseTimestamp.value, isDebugMode.value
  );

  // The plot bubble is already injected at its real position by the server. Only
  // the summary bubble is a client-side synthetic entry, prepended at the top.
  messages.value = summaryMessage.value ? [summaryMessage.value, ...history] : history;
