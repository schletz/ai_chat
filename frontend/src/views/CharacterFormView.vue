<template>
  <div class="header">
    ...
    <div class="header-right">
      ...
      <AsyncButton v-if="isEditMode" type="button" :action="generateSummary" :disabled="isFetching"
        customClass="header-action header-summary">
        Chat zusammenfassen
      </AsyncButton>
    ...
    <div class="box form-box">
      ...
      <form v-else @submit.prevent class="flex-form">
        ...
        <div class="prompt-grid">
          ...
          <div class="form-group">
            ...
            <small class="hint">
              ...
              Die Position gibt an, wie viele User-Nachrichten nach der Note in der History stehen
              sollen: 0 (Standard) sendet die Note als letzten Eintrag an das LLM, 1 setzt sie
              direkt vor die letzte User-Nachricht. Leer lassen entspricht 0.
            </small>
          </div>
        </div>
        ...
        <div class="params-grid">
          ...
          <div class="form-group">
            <label>Anzahl letzter Assistant-Nachrichten</label>
            <input v-model.number="formData.full_history_for_last_n" type="number" min="0" step="1" placeholder="z. B. 1" />
            <small class="hint">
              Sendet nur die letzten n Assistant-Nachrichten, davor ausschließlich
              Usernachrichten; leer lassen, um alle zu senden.
            </small>
          </div>

          <div class="form-group">
            <label>Anzahl letzter Assistant-Nachrichten (Plotgenerierung)</label>
...
/* Header is split into a left (back + title) and right (actions) group so the
   save/summary buttons live in the top bar instead of at the bottom. */
.header-left {
  display: flex;
  align-items: center;
  min-width: 0;
...
.header-summary:hover:not(:disabled) {
