    ...
    <div class="box" style="margin-top: 20px;">
      ...
      <div v-else>
        ...
        <div class="form-group">
          ...
          <input
            v-model.number="nGpuLayers"
            type="number"
            min="-1"
            placeholder="-1 (alle Layer auf GPU)"
          />
          <small class="hint-text">
            -1 = alle Layer auf GPU. Bei 70B-Modellen auf 32 GB VRAM ggf. ~50 setzen.
          </small>
        </div>
      </div>
...
const nGpuLayers = ref<number>(-1);
const originalNGpuLayers = ref<number>(-1);
...
/** Sync the GPU layer input to the currently selected model's persisted value */
watch(selectedModelId, (newId) => {
  const model = availableModels.value.find(m => m.id === newId);
  const layers = model?.n_gpu_layers ?? -1;
  nGpuLayers.value = layers;
  if (newId === originalModelId.value) {
    originalNGpuLayers.value = layers;
  }
});
...
onMounted(async () => {
  ...
  try {
    ...
    const activeModel = availableModels.value.find(m => m.id === originalModelId.value);
    const layers = activeModel?.n_gpu_layers ?? -1;
    nGpuLayers.value = layers;
    originalNGpuLayers.value = layers;
  } catch (error) {
...
const changeModel = async () => {
  ...
  try {
    // Send model selection command and persist GPU layer count to the backend
    const layers = Number.isFinite(nGpuLayers.value) ? nGpuLayers.value : -1;
    await api.post('/system/model', {
    ...
    // Reset to home route and hard reload, preserving any subdirectory deployment
